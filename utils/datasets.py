...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):
            ...
            try:
                print_s = s.split('@')[0].split('/')[0] + '//****:*****@' + s.split('@')[-1]
            except:
                print_s = s
            print(f'{i + 1}/{n}: {print_s}... ', end='')
...
def letterbox(img, new_shape=640, color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32, interpolation=cv2.INTER_LINEAR):
    # Resize and pad image while meeting stride-multiple constraints
    shape = img.shape[:2]  # current shape [height, width]
    assert isinstance(new_shape, int), "New shape must be a single integer!"
    ...
    if shape[::-1] != new_unpad:  # resize
        img = cv2.resize(img, new_unpad, interpolation)
